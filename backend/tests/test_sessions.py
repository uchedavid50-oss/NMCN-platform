...
def _signup_and_login(client, email, user_agent=UA_CHROME_WINDOWS):
...
def test_login_creates_a_session(client, db_session):
    ...
    response = _signup_and_login(client, email)
...
def test_third_device_login_is_blocked(client):
    email = "device2@example.com"
    first = _signup_and_login(client, email, user_agent=UA_CHROME_WINDOWS)
    second = _signup_and_login(client, email, user_agent=UA_SAFARI_IPHONE)
    third = _signup_and_login(client, email, user_agent=UA_FIREFOX_MAC)
...
def test_logout_frees_a_device_slot(client):
...
def test_logged_out_token_no_longer_authenticates(client):
    email = "device4@example.com"
    login_response = _signup_and_login(client, email)
...
def test_list_sessions_shows_devices_with_current_flagged(client):
...
def test_delete_specific_session_logs_out_that_device(client):
...
def test_cannot_delete_another_users_session(client, make_user):
    email = "device7@example.com"
    owner_login = _signup_and_login(client, email, user_agent=UA_CHROME_WINDOWS)
