...
def _signup_and_login(client, db_session, email="2fa-test@example.com", password="testpass1"):
...
def test_2fa_setup_returns_secret_and_uri_but_does_not_enable_yet(client, db_session):
    headers, _, _ = _signup_and_login(client, db_session)
...
def test_2fa_verify_enables_it_with_correct_code(client, db_session):
    headers, _, _ = _signup_and_login(client, db_session)
...
def test_2fa_verify_rejects_wrong_code(client, db_session):
    headers, _, _ = _signup_and_login(client, db_session)
...
def test_login_requires_2fa_code_once_enabled(client, db_session):
    headers, email, password = _signup_and_login(client, db_session)
    setup = client.post("/auth/2fa/setup", headers=headers).json()
    code = pyotp.TOTP(setup["secret"]).now()
...
def test_login_rejects_wrong_2fa_code(client, db_session):
    headers, email, password = _signup_and_login(client, db_session)
    setup = client.post("/auth/2fa/setup", headers=headers).json()
    code = pyotp.TOTP(setup["secret"]).now()
    client.post("/auth/2fa/verify", json={"code": code}, headers=headers)
...
def test_2fa_disable_requires_valid_code(client, db_session):
    headers, _, _ = _signup_and_login(client, db_session)
    setup = client.post("/auth/2fa/setup", headers=headers).json()
    code = pyotp.TOTP(setup["secret"]).now()
    client.post("/auth/2fa/verify", json={"code": code}, headers=headers)
